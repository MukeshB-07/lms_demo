...
pipwerks.SCORM.API.get = function(){
    ...
    if(!API && win.opener){
        API = find(win.opener);
        if(!API && window.opener.parent){
            for (var i=0;i<window.opener.parent.frames.length;i++){
                API = find(window.opener.parent.frames[i]);
                if(API) break;
            }
        }
    }
    if(!API && win.opener && win.opener.opener){
        API = find(window.opener.opener);
        if(!API && window.opener.opener.parent){
            API = find(window.opener.opener.parent);
            if(!API){
                for (var j=0;j<window.opener.opener.parent.frames.length;j++){
                    API = find(window.opener.opener.parent.frames[j]);
                    if(API) break;
                }
            }
        }
...
pipwerks.SCORM.data.set = function(parameter, value){
    ...
    //console.log("api set, scorm.connection.isActive: ", scorm.connection.isActive);
    if(scorm.connection.isActive){

        var API = scorm.API.getHandle(),
            errorCode = 0;
            //console.log("api set, API: ", API);
        if(API){
            //console.log("api set, API found, scorm.version: ", scorm.version);
            switch(scorm.version){
                case "1.2" : success = makeBoolean(API.LMSSetValue(parameter, value)); break;
                case "2004": success = makeBoolean(API.SetValue(parameter, value)); break;
            }
            //console.log("api set, success: ", success);
            if(success){
